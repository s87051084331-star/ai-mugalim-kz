...
def init_db():
    with db() as c:
        with c.cursor() as cur:
            ...
            cur.execute("""CREATE TABLE IF NOT EXISTS shared_kmj(
              id BIGSERIAL PRIMARY KEY,
              author_id BIGINT REFERENCES users(id) ON DELETE CASCADE,
              subject TEXT NOT NULL, grade TEXT DEFAULT '', quarter TEXT DEFAULT '', section TEXT DEFAULT '',
              topic TEXT NOT NULL, learning_objective TEXT DEFAULT '', title TEXT NOT NULL,
              kmj_json JSONB NOT NULL DEFAULT '{}'::jsonb, status TEXT NOT NULL DEFAULT 'published',
              uses INTEGER NOT NULL DEFAULT 0, created_at TIMESTAMPTZ DEFAULT NOW()
            )""")
            cur.execute("""CREATE TABLE IF NOT EXISTS task_bank(
              id BIGSERIAL PRIMARY KEY,
              author_id BIGINT REFERENCES users(id) ON DELETE SET NULL,
              subject TEXT NOT NULL, grade TEXT DEFAULT '', quarter TEXT DEFAULT '', section TEXT DEFAULT '',
              topic TEXT DEFAULT '', learning_objective TEXT DEFAULT '', task_type TEXT DEFAULT 'exercise',
              work_type TEXT DEFAULT 'individual', level TEXT DEFAULT '', question TEXT NOT NULL,
              answer TEXT DEFAULT '', solution TEXT DEFAULT '', descriptor TEXT DEFAULT '', points INTEGER DEFAULT 1,
              is_shared BOOLEAN NOT NULL DEFAULT TRUE, created_at TIMESTAMPTZ DEFAULT NOW()
            )""")
...
САПА ТАЛАПТАРЫ:
- Пәнге бейімдел: математикада нақты есептер мен өрнектерді, физикада формула/өлшем бірлігін, химияда реакция/есепті, тіл пәндерінде оқылым-жазылым-айтылым тапсырмаларын нақты бер.
- «Есеп шығарады» деп жалпылама жазба: тапсырманың өзін толық жаз.
- Сабақта кемінде жеке, жұптық не топтық нақты тапсырмалар болсын; қажет жерде A/B/C деңгейін қолдан.
- Әр тапсырмаға өлшенетін дескриптор және балл бер.
- Сабақ соңында нақты бекіту тапсырмасы, рефлексия және нақты үй тапсырмасы болсын.
- Жауап/шешім мұғалімге арналған мәтінде көрсетілсін, бірақ оқушы әрекетінде дайын жауап берілмесін.
- Кезеңдердің минуттарының қосындысы сабақ ұзақтығына сәйкес келсін.
Тек JSON қайтар:
...
class SharedKmjBody(BaseModel):
    title:str; subject:str; grade:str=""; quarter:str=""; section:str=""; topic:str=""; learning_objective:str=""; kmj:dict

class BankTaskBody(BaseModel):
    subject:str; grade:str=""; quarter:str=""; section:str=""; topic:str=""; learning_objective:str=""
    task_type:str="exercise"; work_type:str="individual"; level:str=""; question:str; answer:str=""; solution:str=""; descriptor:str=""; points:int=1; is_shared:bool=True

@app.get("/api/library/kmj")
def library_kmj(request:Request,subject:str="",grade:str="",q:str=""):
    auth_user(request)
    where=["s.status='published'"]; args=[]
    if subject: where.append("LOWER(s.subject)=LOWER(%s)");args.append(subject)
    if grade: where.append("s.grade=%s");args.append(grade)
    if q: where.append("(s.topic ILIKE %s OR s.learning_objective ILIKE %s OR s.title ILIKE %s)");args += [f"%{q}%"]*3
    with db() as c:
        with c.cursor() as cur:
            cur.execute("SELECT s.id,s.title,s.subject,s.grade,s.quarter,s.section,s.topic,s.learning_objective,s.uses,s.created_at,u.name author FROM shared_kmj s JOIN users u ON u.id=s.author_id WHERE "+" AND ".join(where)+" ORDER BY s.id DESC LIMIT 100",args); rows=cur.fetchall()
    return {"items":rows}

@app.post("/api/library/kmj/publish")
def library_kmj_publish(body:SharedKmjBody,request:Request):
    u=auth_user(request)
    with db() as c:
        with c.cursor() as cur:
            cur.execute("""INSERT INTO shared_kmj(author_id,subject,grade,quarter,section,topic,learning_objective,title,kmj_json) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING id""",(u["id"],body.subject,body.grade,body.quarter,body.section,body.topic,body.learning_objective,body.title,json.dumps(body.kmj,ensure_ascii=False))); rid=cur.fetchone()["id"]
    return {"ok":True,"id":rid}

@app.get("/api/library/kmj/{item_id}")
def library_kmj_get(item_id:int,request:Request):
    auth_user(request)
    with db() as c:
        with c.cursor() as cur:
            cur.execute("UPDATE shared_kmj SET uses=uses+1 WHERE id=%s",(item_id,));cur.execute("SELECT * FROM shared_kmj WHERE id=%s",(item_id,));row=cur.fetchone()
    if not row: raise HTTPException(404,"ҚМЖ табылмады")
    return {"item":row}

@app.get("/api/library/tasks")
def library_tasks(request:Request,subject:str="",grade:str="",q:str=""):
    auth_user(request);where=["is_shared=TRUE"];args=[]
    if subject:where.append("LOWER(subject)=LOWER(%s)");args.append(subject)
    if grade:where.append("grade=%s");args.append(grade)
    if q:where.append("(topic ILIKE %s OR learning_objective ILIKE %s OR question ILIKE %s)");args += [f"%{q}%"]*3
    with db() as c:
        with c.cursor() as cur:cur.execute("SELECT * FROM task_bank WHERE "+" AND ".join(where)+" ORDER BY id DESC LIMIT 150",args);rows=cur.fetchall()
    return {"items":rows}

@app.post("/api/library/tasks")
def library_task_add(body:BankTaskBody,request:Request):
    u=auth_user(request)
    with db() as c:
        with c.cursor() as cur:
            cur.execute("""INSERT INTO task_bank(author_id,subject,grade,quarter,section,topic,learning_objective,task_type,work_type,level,question,answer,solution,descriptor,points,is_shared) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING id""",(u["id"],body.subject,body.grade,body.quarter,body.section,body.topic,body.learning_objective,body.task_type,body.work_type,body.level,body.question,body.answer,body.solution,body.descriptor,max(0,body.points),body.is_shared));rid=cur.fetchone()["id"]
    return {"ok":True,"id":rid}
