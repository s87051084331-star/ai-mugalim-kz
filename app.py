...
def zerek_class_student_delete(class_id:int,student_id:int,request:Request):
    u=auth_user(request)
    with db() as c:
        with c.cursor() as cur:
            _zerek_owned_class(cur,class_id,u["id"])
...
async def zerek_class_import(class_id:int,request:Request,file:UploadFile=File(...)):
